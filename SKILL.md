...
Asks once, at the start of a conversation, which language to work in — 🇫🇷 French · 🇬🇧 English · 🇪🇸 Spanish · 🇸🇦 Arabic — then stays in that language for the rest of the engagement.

Designed for efficiency: the always-loaded `SKILL.md` stays lean (~17.7K chars) by pushing detailed protocols, terminology tables, and program guides into reference files loaded only when the task needs them.
...
| 🗳️ Decision Thinking (i4Policy) | ADDIS — Agenda Setting, Drafting, Decision-Making, Implementation, Sense-making |
...
└── references/                       ← loaded on demand
    ...
    ├── addis-methodology.md          ← ADDIS decision thinking, foresight-ADDIS bridge
    └── multilingual-terms.md         ← FR/EN/ES/AR terminology (loaded on demand)
```
...
## Decision Thinking overlay (ADDIS)

For engagements that must end in an **adopted institutional decision** — public policy,
multi-stakeholder consortium, citizen co-creation — the skill can overlay the ADDIS
methodology (i4Policy / decisionthinking.org) on top of the consulting program:

`Agenda Setting → Drafting → Decision-Making → Implementation → Sense-Making`

Each ADDIS step maps to a specific foresight method (e.g. scenarios feed Ideate,
Wind Tunneling output feeds Propose, the Early Warning System feeds Monitor),
turning a foresight report into a governed, legitimate, monitored decision.
